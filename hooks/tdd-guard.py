#!/usr/bin/env python3
"""PreToolUse hook: enforce test-first inside a locked story.

While a story is active and a failing test has not yet been observed, deny edits to its
non-test contract files. Test files stay editable so the test can be written first. The
implementer clears the gate with `scrum_state.py mark-red` after seeing red. Fail-open.
"""
import json
import os
import re
...
def is_test_file(path):
    return bool(TEST_PATH.search(path.replace(os.sep, "/")))
...
def main():
    ...
    if not story or story.get("red_test_observed"):
        return
    ...
    if target not in set(story.get("files", [])) or is_test_file(target):
        return
    _deny(
        f"TDD: write a failing test for story '{story.get('id', '?')}' first. Edit the test, run "
