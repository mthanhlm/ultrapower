...
Init runs automatically on entry: the router skill (`ultrapower:run`) injects
`skills/run/scripts/codegraph-ensure.sh`, keyed off the **filesystem at the current
...
can answer for. When `<root>/.codegraph/` is **absent** and CodeGraph is available it
ensures `.codegraph/` is in `<root>/.gitignore` (append if missing — `codegraph init`
does not add it), then runs `codegraph init <root>` (local, safe, reversible).
`ultrapower:codegraph` carries the same logic as a manual fallback. Pin MCP queries
to this tree with `projectPath: <root>` so a same-named repo can't answer for it. After `ultrapower:implement` finishes changing files, run
